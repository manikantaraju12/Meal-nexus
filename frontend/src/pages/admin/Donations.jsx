...
const AdminDonations = () => {
  ...
  const counts = Object.fromEntries(STATUSES.map(s => [s, s === 'all' ? donations.length : donations.filter(d => d.status === s).length]));
    ...
    <Layout title="All Donations">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-5">
        <input type="text" value={search} onChange={e => setSearch(e.target.value)}
          className="glass-input" placeholder="Search food type, donor, city..." style={{ maxWidth: '18rem' }}
        />
      ...
      <div className="glass-card">
        {filtered.length === 0 ? (
          <div className="text-center py-16 text-white/30">No donations found</div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-1 text-xs text-white/50">
                      <span><span className="text-white/25 uppercase tracking-wide mr-1">Qty</span>{d.foodDetails?.quantity?.value} {d.foodDetails?.quantity?.unit}</span>
                      <span><span className="text-white/25 uppercase tracking-wide mr-1">City</span>{d.pickupLocation?.city || '—'}</span>
                      <span><span className="text-white/25 uppercase tracking-wide mr-1">Donor</span>{d.donor?.name || '—'}</span>
                      <span><span className="text-white/25 uppercase tracking-wide mr-1">NGO</span>{d.assignedNGO?.name || '—'}</span>
                      <span><span className="text-white/25 uppercase tracking-wide mr-1">Volunteer</span>{d.assignedVolunteer?.name || 'Unassigned'}</span>
                      <span><span className="text-white/25 uppercase tracking-wide mr-1">Posted</span>{new Date(d.createdAt).toLocaleDateString('en-IN', { dateStyle: 'medium' })}</span>
