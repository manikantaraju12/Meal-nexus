...
const NGODonations = () => {
  ...
  const fetchDonations = async () => {
    try {
      ...
      const mine = res.data.filter(d =>
        d.assignedNGO?._id === user?._id || d.assignedNGO?.id === user?._id
      );
      setDonations(mine);
  ...
  const STATUSES = ['all', 'accepted', 'picked', 'delivered'];
  ...
  if (loading) return (
    <Layout title="My Accepted Donations">
      ...
      <div className="glass-card">
        {filtered.length === 0 ? (
          <div className="text-center py-16 text-white/30">
            {donations.length === 0 ? "You haven't accepted any donations yet" : 'No donations in this status'}
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-1 text-xs text-white/50">
                      ...
                      <span>
                        <span className="text-white/25 uppercase tracking-wide mr-1">Donor</span>
                        {d.donor?.name || '—'}
                      </span>
                      <span>
